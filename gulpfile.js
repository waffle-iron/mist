...
console.log('Mist-expanse version:', version);
...
gulp.task('copy-node-folder-files', ['clean:dist'], () => {
    ...
    _.each(osArchList, (osArch) => {
        if (platformIsActive(osArch)) {
            ...
            .pipe(gulp.dest(`./dist_${type}/app/nodes/gexp/${osArch}`)));
        }
    });
...
gulp.task('build-dist', ['copy-i18n'], (cb) => {
    ...
    const appPackageJson = _.extend({}, packJson, {
        ...
        build: {
            ...
            extraFiles: [
                `nodes/gexp/mac-x64`, // eslint-disable-line no-template-curly-in-string
            ],
            linux: {
                target: [
                    'zip',
...
gulp.task('update-nodes', (cb) => {
    ...
    const clientBinariesGeth = clientBinaries.clients.Gexp;
    const localGethVersion = clientBinariesGeth.version;
    const newJson = clientBinaries;
    const geth = newJson.clients.Gexp;

    // Query latest geth version
    got('https://api.github.com/repos/expanse-org/go-expanse/releases/latest', { json: true })
    .then((response) => {
    ...
    .then((tagName) => {
        ...
        if (cmp(latestGethVersion, localGethVersion)) {
            ...
            got(`https://api.github.com/repos/expanse-org/go-expanse/commits/${tagName}`, { json: true })
            .then((response) => {
