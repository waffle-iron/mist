...
On OSX its `~/Library/Expanse/gexp.ipc` on linux `~/.ethereum/gexp.ipc` and on windows it uses a named pipe, which doesn't need to be renamed.

Example:

    $ geth --datadir /my/chain/ --networkid 23 --ipcpath ~/Library/Expanse/gexp.ipc
